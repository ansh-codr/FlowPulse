...
exports.dailyAggregation = functions.pubsub
    ...
    for (const userDoc of usersSnap.docs) {
        ...
        const mobileSummary = mobileSummarySnap.data() ?? {};
        ...
        const balancedLearningAndMovement = totalDuration >= 2 * 3600 &&
            productiveTime >= totalDuration * 0.5 &&
            mobileActiveMinutes >= 30 &&
            mobileStepCount >= 6000;
        const highScreenUsageLowPhysicalActivity = desktopScreenTimeMinutes >= 240 &&
            (dailyStepCount < 3000 || activeMovementMinutes < 20);
        const healthyLearningMovementBalance = learningActivityMinutes >= 120 &&
 ...
 */
function countLongSedentaryStudyPeriods(logs) {
